...
| `VERIFICA_PRECIOS.gs` | `VERIFICA_PRECIOS` | Confirma en el backend real que el precio de venta se convierte con `TIPO CAMBIO` (RULE-REP-018), que es el fix que sigue **sin verificar de punta a punta**. Es de **solo lectura**: llama a `PP_fetchRestletPages_` y a `PP_fetchSalesPricesRestlet_`, que solo hacen `UrlFetch` de lectura al 1766; no llama a `syncNetSuiteWorkOrdersLite` ni a `syncNetSuiteData`. Hace 5 pasos: (1) que haya credenciales `NS_*` sin imprimirlas; (2) que el 1766 responda y de cuántas filas, en cuántos segundos; (3) **volca las columnas crudas** de las ventas en moneda extranjera —`PRECIO BASE MNX`, `TIPO CAMBIO`, `MONEDA`— porque el campo `PRECIO BASE MNX` miente el nombre: viene en la moneda de la transacción; (4) aplica la misma regla del servidor (`precio = crudo x tipoCambio`) y muestra el antes y el después con el **factor mediano**, que es el veredicto: si anda cerca de 18, la conversión está viva; (5) **cruza con la función real del proyecto** (`PP_fetchSalesPricesRestlet_`, sin caché) y dice si lo que devuelve coincide con el cálculo, para probar que el fix está en el camino que la app usa de verdad y no solo en la aritmética de la sonda. **Correla una vez, no en bucle**: el 1766 son 6 671 filas con tope real de `pageSize` 1000, o sea 7 llamadas por recorrido, y la cuenta es de consumidor con 20 000 `UrlFetch`/día. |
