...
| `DIAG_operaciones2240b.gs` | `DIAG_operaciones2240b` | **Correrse ANTES de subir el 2240 corregido.** Arma a mano el mismo SQL que lleva el RESTlet y lo manda por la API REST de SuiteQL (el mismo camino que el catálogo maestro en producción, con `Prefer: transient`), sin depender del 2240. Verifica tres cosas riesgosas: (1) que la cuenta **acepte `FETCH NEXT`/`OFFSET`** y que el `OFFSET` avance de verdad (si no, no subas la versión paginada); (2) que **`tl.location = 1` devuelva filas** (si devuelve 0, la columna no se llama así en esta cuenta y no subas el filtro); (3) el desglose de operaciones abiertas por ubicación, que confirma que la columna existe. Incluye guardia de cuota. |
