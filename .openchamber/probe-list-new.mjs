...
const result = await page.evaluate(() => (async () => {
  const out = { backend: "" };
  if (typeof PPAppsScriptBridge !== "undefined") {
    out.backend = PPAppsScriptBridge.defaultWebAppUrl || PPAppsScriptBridge.DEFAULT_WEB_APP_URL || "?";
  }
  ...
  try {
    ...
    out.result = Array.isArray(res) ? res.map((s) => ({ id: s.snapshotId, week: s.weekStart, start: s.planStart, ops: s.operations, gen: s.generatedAt, label: s.label, status: s.status })) : res;
