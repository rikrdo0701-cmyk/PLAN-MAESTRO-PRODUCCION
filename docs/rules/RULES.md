...
| `RULE-REP-013` | Pestaña "Liberación final" en Reportes: lista OTs con operación 16OC/39OTD una fila por OT, columnas OT, Artículo, Cantidad, Fecha (inicio de la operación de liberación); filas ordenadas ascendente por fecha de plan de la operación (más antiguo a más nuevo); exporta `liberacion-final-16oc-39otd.xlsx` con `buildXlsxBytes`. Detección `finalReleaseOperationMatch`: ct 5504/5537 → si ct vacío/SIN_CT matchea código 16OC/39OTD en `descripcion` (feed trae SIN_CT) → fallback `capabilityForOperation`; ct real distinto nunca matchea | IMPLEMENTADA | definición de usuario 2026-09-23; FIX 0 OT con Borrador (SIN_CT); `index.template.html` (`data-tab="release"`), `app.js` (`releaseReportRows`/`finalReleaseOperationMatch`/`exportReleaseXlsx`); tests 554/554 |
