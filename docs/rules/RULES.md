...
| ID | Nombre | Estado | Fuente |
|---|---|---|---|
| `RULE-PERF-001` | Los payloads de publicaci\u00f3n/guardado excluyen `machineToolHistory` (dato derivado del servidor de `PLANES_HISTORICOS`, no escrito desde el payload): publish baja de 2,077,555 B a ~1.51 MB; se conservan `operations`/`workOrders`/`materials`; el snapshot ya no persiste la copia de `machineToolHistory` que viajaba via `persistableState` y al restaurar se re-deriva por `PP_readState_` | IMPLEMENTADA | PLAYWRIGHT producci\u00f3n 2026-09-10 (`probe-perf-prod-2026-09-10.mjs`), `02-storage.js:227/630/1270-1292`, FIX 2026-09-10 `app.js` (`publishCurrentPlan`/`createAppSheetPayload`), `tests/build.test.mjs` |
