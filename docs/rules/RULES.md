...
| `RULE-PERF-005` | Reportes/PDF/publish: plan de bajo riesgo DOCUMENTADO (NO implementado). Medicion ANTES headless BLOQUEADA reproduciblemente 2026-09-12 (Playwright local site/ :8787): bridge Apps Script `{unchanged:true}` (stale-while-revalidate `15-performance-service.js:` 82) + `compactLocalState` retiran `operations` -> ops SIEMPRE 0 en boot limpio; una corrida aislo wo=232/cards=170 reales pero ops=0 (sembrar/capturar prohibido). Sin baseline ANTES no se toca codigo (RULE-GOV-004); intento de memo de `reportOperationsSource` roto por `tests/build.test.mjs:1642` y REVERTIDO (app.js identico a HEAD, git diff vacio). Objetivo ya cubierto por RULE-PERF-001/002/003/004. PLAN PENDIENTE (patron RULE-PERF-003): memo semanal `renderReports`/`renderWeekReport` (app.js:6715/6727) reusada en `generatePlanPdf` (5897) y publish cliente (5805/11269), sin tocar server publish/scheduler | DOCUMENTADA | auditoria 2026-09-12 + revert de app.js (build.test.mjs:1642), suite 503/503 PASS, build OK |
