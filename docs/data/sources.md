...
- **El 2240 (CORREGIDO 2026-09-26, se sube a NetSuite a mano):** el archivo del repo ya trae las tres correcciones de RULE-REP-019, pero **producción sigue con la versión vieja** hasta que se suba. Antes: `const all = runSuiteQL_(sql)` + `all.slice(from, to)`, o sea que **cada llamada re-ejecutaba el JOIN completo** de `manufacturingoperationtask` + `transaction` + `transactionline`, y `post()` **ignoraba `body.locationId`** (solo leía `pageSize` y `pageIndex`). Ahora: página en SQL con `FETCH NEXT`/`OFFSET`, `tl.location = ?` como parámetro ligado y `location` en la respuesta. El desglose medido que motiva el filtro: **2231** operaciones de planta 1 contra **169** de otras (7%).
  - **Riesgo y cómo se cubre:** `FETCH NEXT/OFFSET` es sintaxis SuiteQL y `tl.location` puede no llamarse así en la cuenta. La sonda `DIAG_operaciones2240b` demostró (2026-09-26 07:31) que el endpoint REST de SuiteQL **no reproduce** el `query.runSuiteQL` de dentro de NetSuite — falló con un SQL que el 2240 de producción ejecuta bien. Por eso el RESTlet lleva un **fallback de estrategias** y subirlo nunca deja la app peor: `sql-paginado+ubicacion` → `sql-paginado` → `sql-completo+ubicacion` → `sql-completo` (este último es el camino viejo, que pagina en memoria). Si una lanza excepción se pasa a la siguiente; si el filtro por ubicación devuelve **0 filas sin excepción** (el peor caso: la columna existe pero descarta todo), también degrada a sin filtro. `debug` deja constancia de qué estrategia respondió y por qué se degradó. Si fallan las cuatro, el error enumera los intentos.
