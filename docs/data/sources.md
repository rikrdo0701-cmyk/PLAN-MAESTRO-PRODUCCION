...
- **Tiempos (RULE-REP-016):** `fetchNetSuiteWorkOrdersLite` mide 43–73 s porque `PP_fetchSalesPricesRestlet_` pagina el `1766` `REQ_FIFO` hasta 100 páginas de 200 filas y filtra la ventana de 6 meses **en memoria**, no en la petición (el body solo manda `table`). Cada petición que sufra el límite de solicitudes añade hasta 17 s. El presupuesto del cliente es 180 s por intento (`NETSUITE_BACKLOG_SYNC_TIMEOUT_MS`) y el del puente 420 s para `fetchNetSuiteWorkOrdersLite`/`syncNetSuiteWorkOrdersLite`, que antes caían en el genérico de 120 s y cortaban antes que el cliente. Reducir la duración real exige tocar el `1766` en NetSuite (ventana de fechas en la petición o cachear el resultado), cuyo fuente **no está en este repositorio**.
