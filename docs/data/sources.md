...
- **El 2240 pagina en memoria** (`netsuite-restlet-operaciones.js:64-68`: `all = runSuiteQL_(sql)` y después `all.slice(from, to)`), así que **cada llamada re-ejecuta el JOIN completo** de `manufacturingoperationtask` + `transaction` + `transactionline`. Con `pageSize` 200 eran 12 escaneos completos; con 2500 es uno. El mismo defecto ya se corrigió en el 2244 con `FETCH NEXT/OFFSET` (RULE-REP-016-A); el 2240 sigue con el patrón viejo y su corrección queda pendiente.
- **El 2240 no expone columna de ubicación ni planta** (verificado agrupando por `location_id`, `locationId`, `location`, `planta`, `PLANTA`, `plant`, `location_name`, `subsidiary`: ninguno existe) y su `post()` **ignora `body.locationId`** (solo lee `pageSize` y `pageIndex`). El filtro de planta se aplica hoy **en memoria** en `PP_belongsToPlant_` (`08-netsuite.js:833`) después de traer todo. Desglose medido: **2231** operaciones de planta 1, **169** de otras plantas (7%), 0 sin folio, sobre 2400. Bajar el filtro al SQL exige **agregar la columna al `SELECT`** y subir el RESTlet a NetSuite; con 7% de ahorro no compensa.
